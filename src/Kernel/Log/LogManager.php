...
namespace OaTapdApi\Kernel\Log;
...
class LogManager implements LoggerInterface
{
    ...
    {
        return new Monolog('OaTapdApi', $this->prepareHandlers([new StreamHandler(
            \sys_get_temp_dir().'/oa_tapd_api/oa_tapd_api.log', $this->level(['level' => 'debug'])
        )]));
    }
    ...
    protected function createSyslogDriver(array $config)
    {
        return new Monolog($this->parseChannel($config), [
            $this->prepareHandler(new SyslogHandler(
                    'CodingOpenApi', $config['facility'] ?? LOG_USER, $this->level($config))
            ),
        ]);
    }
    ...
    {
        return ($this->app['config']['log']['default']) ?? 'single';
    }
